public class FancyRunsort
{
	...
	public static void sort(Comparable[] a) {
        ...
        for(int pointer = 0; pointer < N; pointer++){
        	...
        	if(runEnd == N){		
                if(runs == 2){
        		    //StdOut.println("firstRunStart: "+firstRunStart+" - runEnd: "+(runEnd));
                    //StdOut.println("Runlength: "+(runEnd-firstRunStart));
                    int runlength = runEnd-firstRunStart;
                    if(runlength < 8){
                        //StdOut.println("insertionsort");
                        insertionsort(a, firstRunStart, runEnd);
                    }else{
